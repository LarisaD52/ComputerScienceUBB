...
public class ProgramExecutorController {
    ...
    @FXML
    private TableView<SemaphoreTableRow> semaphoreTableView;

    @FXML
    private TableColumn<SemaphoreTableRow, Integer> semIndexColumn;

    @FXML
    private TableColumn<SemaphoreTableRow, Integer> semValueColumn;

    @FXML
    private TableColumn<SemaphoreTableRow, String> semListColumn;
    ...
    public void initialize() {
        ...
        // Listener pentru a schimba datele afișate când selectezi alt ID de program
        ...
        semIndexColumn.setCellValueFactory(
                new PropertyValueFactory<>("index")
        );
        semValueColumn.setCellValueFactory(
                new PropertyValueFactory<>("value")
        );
        semListColumn.setCellValueFactory(
                new PropertyValueFactory<>("list")
        );

    }

    // FIX: selectează ProgramState după ID-ul selectat, nu după index
    ...
    private void populateProgramStateIdentifiersListView() {
        ...
        // FIX: dacă nu e nimic selectat, selectează primul
    ...
    private void populateSemaphoreTable(ProgramState state) {
        List<SemaphoreTableRow> rows = new ArrayList<>();

        state.getSemaphoreTable().getContent().forEach((key, value) -> {
            Integer permits = value.getKey();
            String list = value.getValue().toString();
            rows.add(new SemaphoreTableRow(key, permits, list));
        });
    ...
    private void runOneStep(ActionEvent actionEvent) {
        ...
        try {
            ...
            if (!prgList.isEmpty()) {
                ...
                populate();   // IMPORTANT: afișăm starea actuală

                // dacă nu mai există execuție activă
